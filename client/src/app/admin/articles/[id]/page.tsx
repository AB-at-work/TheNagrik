...
export default function ArticleEditorPage() {
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      api.get<Article>(`/articles/${params.id}`)
        .then((data) => {
          setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (isNew) {
        await api.post('/articles', formData, { auth: true });
      } else {
        await api.put(`/articles/${params.id}`, formData, { auth: true });
      }
      router.push('/admin/articles');
    } catch (err: any) {
      alert(err.details?.[0]?.message || err.message || 'Error saving article');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>{isNew ? 'Write Article' : 'Edit Article'}</h1>
        <Button variant="secondary" onClick={() => router.push('/admin/articles')}>Back</Button>
      </div>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.sideColumn}>
          ...
          <div className={styles.actions}>
            <Button type="submit" isLoading={saving}>Save Article</Button>
          </div>
        </div>
      </form>
    </div>
